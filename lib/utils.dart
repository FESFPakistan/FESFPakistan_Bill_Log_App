...
class Utils {
  ...
  static Future<double?> fetchBalance(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (decoded is Map && decoded.containsKey('current_balance')) {
          return (decoded['current_balance'] as num?)?.toDouble() ?? 0.0;
        }
        return 0.0;
      } else {
        await showSnackBar(context, 'Failed to refresh balance: ${response.statusCode}');
        return null;
      }
    } catch (e) {
      await showSnackBar(context, 'Error refreshing balance: $e');
      return null;
    }
  }
  ...
  static Future<Map<String, dynamic>> fetchReportingPeriod(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map && data.containsKey('active_reporting_period')) {
          ...
          await showSnackBar(context, 'Invalid reporting period data.');
          return {
            'period': 'Invalid data',
            'openingBalance': null,
            'formattedStartDate': 'N/A',
        ...
        await showSnackBar(context, 'Failed to load reporting period: ${response.statusCode}');
        return {
      ...
      await showSnackBar(context, 'Error loading reporting period: $e');
      return {
        'period': 'Error: $e',
        'openingBalance': null,
        'formattedStartDate': 'N/A',
  ...
  static void showBillDetails(BuildContext context, Map<String, dynamic> bill) {
    try {
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Narration: ${bill['narration'] ?? ''}',
                    style: GoogleFonts.montserrat(
                        fontSize: getResponsiveFontSize(context, 14.0), fontWeight: FontWeight.w500)),
                Text('Amount: Rs. ${NumberFormat('#,###').format((bill['amount'] as num).round())}',
                    style: GoogleFonts.montserrat(
                        fontSize: getResponsiveFontSize(context, 14.0), fontWeight: FontWeight.w500)),
      ...
      showSnackBar(context, 'Error displaying bill details: $e');
    }
  }
            ...
            child: Row(
              ...
              children: [
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          GestureDetector(
                            onLongPress: () {
                              // This only changes the local variable, not the state.
                              // To update UI, you should use setState in the parent widget.
                ...
                Column(
                  ...
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Rs. ${NumberFormat('#,###').format((bill['amount'] as num).round())}',
                        style: GoogleFonts.montserrat(
                            fontSize: getResponsiveFontSize(context, 12.0),
                            fontWeight: FontWeight.w500)),
                    if (bill['imagePath'] != null)
  ...
  static Future<Map<String, dynamic>> loadExpenseHeads(BuildContext context) async {
    ...
    if (cachedExpenseHeads != null) {
      ...
      if (decoded is List) {
        expenseHeads = List<Map<String, dynamic>>.from(decoded);
    ...
    if (await hasInternetConnection() && (DateTime.now().day == 1 || DateTime.now().day == 16)) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          await showSnackBar(context, 'Failed to load expense heads: ${response.statusCode}');
        }
      } catch (e) {
        await showSnackBar(context, 'Error loading expense heads: $e');
      }
    } else if (cachedExpenseHeads == null) {
      await showSnackBar(context, 'No internet connection. Expense heads not available.');
    }
